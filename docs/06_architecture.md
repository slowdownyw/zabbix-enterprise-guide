...
- Нет **runbook'ов** → дежурный гуглит в 3 ночи
...
- **Нормативно.** Для объектов КИИ стоковый open-source Zabbix не закрывает обязательные меры приказа ФСТЭК №239. Для значимых объектов ЗОКИИ существуют сертифицированные сборки (например, UDV ITM на базе Zabbix).
...
**Где Zabbix уместен в OT:** верхние уровни (Historians, инженерные станции, серверы SCADA на Windows) — по согласованию с ИБ, с отдельным Proxy в DMZ и белым списком метрик. Контроллеры и АРМ L0–L2 — только пассивно: ICMP, SNMP, TCP до OPC-сервера, без агента.
...
На каждой площадке / каждом изолированном сегменте — свой прокси. Балансировщика нет, агент знает один прокси.

**Postgres + Patroni:**

- HA через Patroni + etcd/consul
- WAL-G для backup в S3-совместимое хранилище
- Партиционирование таблиц `history*` обязательно (TimescaleDB или нативное)
- Отдельный read-replica для Grafana (чтобы дашборды не клали БД)

**Grafana:**

- Только для **визуализации**, не для алертинга
- Использует Zabbix как datasource (есть официальный плагин)
...
ИНЦИДЕНТ:         Acknowledge → Диагностика → Resolve → Постмортем
                                          ↓
ОБРАТНАЯ СВЯЗЬ:   Постмортем → обновление шаблонов / порогов / runbook'ов → обратно в Сбор
```
