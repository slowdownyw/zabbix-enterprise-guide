...
- Кому слать алерт (через action conditions по тегам)
- Где показывать ([дашборды по сервисам, не по железкам](01_service_not_host.md))
...
```text
Host groups = права / техническое дерево / грубая навигация
...
| Где тег               | Зачем                                                                  |
| --------------------- | ---------------------------------------------------------------------- |
| **Host tag**          | “Кто этот хост”: сервис, владелец, площадка, среда, критичность        |
...
Самая важная часть: **problem event наследует теги по цепочке** — от template, host, item/web scenario и trigger; одинаковые `tag:value` после разрешения макросов склеиваются, чтобы не было дублей. ([Zabbix][1])
...
Для item-level дашбордов это тоже работает: Item navigator widget умеет фильтровать items по host tags и item tags, а также группировать по значению host tag или item tag — например, по `service`, `location`, `component`. ([Zabbix][2])
...
Host groups остаются основой для нормального RBAC по объектам, но теги могут ограничивать видимость problems для user groups: Zabbix описывает сценарий, где пользователи группы видят только проблемы с определёнными tags. ([Zabbix][1])
...
Это нужно для дашбордов уровня “покажи мне все capacity-метрики по сервису 1С”:
...
Trigger tag — это “смысл проблемы”.
...
Все хосты, на которые навесили template, начинают порождать problems с этими tags. Это не заменяет host tags `service/owner/location`, но закрывает техническую классификацию.
...
Для нормальной промышленной модели host tags должны жить не в голове админа и не в UI, а в декларативном описании хоста:
...
```text
Хост-группы отвечают: кто имеет доступ и где объект технически лежит.
