...
Самая важная часть: **событие PROBLEM наследует теги по цепочке** — от template, host, item/web scenario и trigger; одинаковые `tag:value` после разрешения макросов склеиваются, чтобы не было дублей. ([Zabbix][1])
