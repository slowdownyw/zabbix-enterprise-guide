---
title: "Сервис, а не хост"
description: "Глава показывает, почему объектом мониторинга должен быть сервис, а не отдельный хост, и как это меняет модель событий, ответственности и эксплуатации."
---
...
Между состоянием инфраструктурных компонентов и состоянием бизнес-сервиса всегда есть **разрыв**. Мониторинг, который игнорирует этот зазор, обречён на классическую драму:

- Зам.директора звонит руководителю ИТ: «1С не работает, бухгалтерия стоит!»
- Руководитель ИТ открывает Zabbix: «У нас всё зелёное».
- Через 40 минут выясняется, что закончились доступные лицензии 1С/HASP, веб-публикация падает, но сам сервер пингуется отлично.
...
| **Middleware** | MSSQL, PostgreSQL, Exchange DAG, IIS, веб-публикации 1С | SQL-чеки, штатные шаблоны |
| **Application** | 1С — очереди, фоновые, лицензии RPHost; SCADA — связь | API, мониторинг логов |
| **Business** | «Зарплата начислится сегодня?», «Документ выпускается?» | синтетика, e2e-проверки |
...
4. **MSSQL** для 1С — отвечает на запросы, нет deadlock выше порога
...
Технически это можно реализовать через **trigger dependencies** в Zabbix, но с важной оговоркой: dependency в первую очередь **удерживает actions/notifications** для зависимых триггеров, а не строит иерархию «сервис → причины» визуально. Сервисная синтетика — это верхнеуровневый симптом, а не первопричина. Если зависимость настроена неаккуратно, можно потерять диагностические компонентные алерты именно тогда, когда они нужны.
...
| Сервис | Синтетика |
|---|---|
| **1С** | Тестовый пользователь логинится в веб-публикацию, проводит пустой документ в тестовой базе, выходит. Засекается время каждого шага. **⚠ Только в тестовой согласованной ИБ/руководством — не в производственной базе напрямую.** |
