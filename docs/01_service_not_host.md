...
- Через 40 минут выясняется, что у одного из RPHost закончились лицензии HASP, веб-публикация падает, но сам сервер пингуется отлично.
...
Большинство Zabbix-инсталляций закрывают слои 1–3, плохо закрывают 4, почти не закрывают 5, и **никогда** не закрывают 6.
...
Технически это делается через **trigger dependencies** в Zabbix: компонентные триггеры зависят от сервисного, и подавляются, пока сервисный активен.
...
| Сервис | Синтетика |
|---|---|
| **1С** | Тестовый пользователь логинится в веб-публикацию, проводит пустой документ, выходит. Засекается время каждого шага |
