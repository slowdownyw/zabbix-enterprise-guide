...
## Официальные термины Zabbix: EN / RU

Термины в интерфейсе Zabbix существуют на двух языках. В книге используются английские названия там, где они являются именем UI-элемента, и русские — в описательном тексте. Таблица для сверки:

| Английский (интерфейс) | Официальный русский | Примечание |
|---|---|---|
| Item | Элемент данных | В Zabbix UI на русском — «элемент данных», не «айтем» |
| Host | Хост | В старых версиях встречался перевод «узел сети», сейчас — «хост» |
| Trigger | Триггер | Прямая транслитерация |
| Problem | Проблема | Активное состояние триггера |
| Template | Шаблон | |
| Action | Действие | |
| Discovery rule | Правило обнаружения | LLD rule |
...
| Service (бизнес-смысл) | Сервис | Бизнес-сервис с точки зрения пользователя или эксплуатации: «1С работает», «почта отправляется». В книге именно этот смысл: «Сервис, а не хост». |
...
| **Host group** | Группа хостов в Zabbix. В этой методике нужна для RBAC и грубой технической навигации, а не для сервисного каталога. | [Теги](03_tags_and_groups.md), [Roadmap](07_implementation_roadmap.md) |
