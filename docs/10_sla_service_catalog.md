---
title: "SLA и сервисный каталог"
description: "Глава разбирает SLA, SLO, OLA и сервисный каталог: как опираться на данные Zabbix, оценивать стоимость девяток и согласовывать ожидания бизнеса."
---
...
В правильном SLA **согласованное заранее окно работ не считается downtime**. Иначе любая нормальная эксплуатация (патчи, обновления) убивает SLA. Ключевое слово — «заранее»: уведомление за N дней, явное согласование с бизнес-владельцем.
...
Вариант B: 99.5% (∼3.6 часа/месяц) ← рекомендуем
  Стоимость: +1 админ-час/неделю на профилактику + апгрейд бэкапов
  
Вариант C: 99.9% (∼43 минуты/месяц)
  Стоимость: дополнительный сервер, второй интернет-канал, +2М рублей капитальных затрат, +500K/год операционных
  
Вариант D: 99.99% (∼4 минуты/месяц)
  Стоимость: второй ЦОД, репликация, гео-балансировка, +20М капитальных затрат, +3M/год операционных
```
...
- ❌ **SLA для сервиса, у которого нет владельца.** Пустая формальность.
...
**Месяц 1–2:** никаких SLA. Только **описание сервиса** для топ-10 сервисов: зафиксировать, что сервис делает для бизнеса, кто им владеет, какие компоненты и зависимости у него есть, как он сейчас работает и как фактически контролируется.

**Месяц 3–5:** измеряем SLI для этих сервисов через synthetic monitoring и накапливаем базовую статистику. На этом этапе не подписываем SLA: сначала нужно понять фактическую доступность сервисов и качество самих проверок.
...
В Zabbix есть штатный механизм **Services + SLA**. Это отдельный слой поверх обычных триггеров и событий, а не просто ещё один дашборд.

- **Zabbix Service** — объект в дереве сервисов: `Services → Services`. Проблемы напрямую привязываются не к любому сервису в дереве, а к дочерним сервисам нижнего уровня — через **Problem tags**. Родительские сервисы получают состояние от дочерних сервисов по заданным правилам расчёта.

- **Service tags** — теги самого сервиса. Они используются для привязки сервиса к SLA и service actions. Это не то же самое, что event tags/problem tags у событий мониторинга.

- **Problem tags** — правила сопоставления проблем с сервисом. Если `PROBLEM`-событие имеет теги, совпадающие с problem tags сервиса, оно влияет на состояние этого сервиса. Если указано несколько problem tags, применяется логика AND: проблема должна совпасть со всеми указанными условиями.

- Состояние сервиса может рассчитываться по дочерним сервисам: например, как самый критичный дочерний статус, как самый критичный статус при проблемах у всех дочерних сервисов, либо по дополнительным правилам — N из M, процент дочерних сервисов, весовые правила.

- **SLA-политика** создаётся отдельно: `Services → SLA`. Она определяет SLO, отчётный период, расписание доступности, часовой пояс и исключаемые периоды недоступности. SLA применяется к сервисам через **service tags**, а не через event tags.

- SLA-отчёт доступен в штатном интерфейсе Zabbix: через раздел `Services → SLA report`, из карточки сервиса, а также через виджет `SLA report` на дашборде.

Это не полная замена внешнему synthetic monitoring: внешние проверки по-прежнему полезны как независимый взгляд на сервис. Но для регулярной SLA-отчётности лучше опираться на штатный механизм Zabbix Services + SLA, а не строить собственный расчёт доступности на скриптах.
---
