...
### Подход 1: «Снизу вверх» (от инфры) ❌

Админ смотрит, что у него есть в инфре, и пишет:
- Сервис «MSSQL»
- Сервис «AD»
...
Потом каждый этот «сервис» декомпозируешь на компоненты. **Каталог = бизнес-перспектива + техническое наполнение**.

### Подход 3: Гибридный (как делают в реальности)

Берёшь топ-10 систем, известных бизнесу, и описываешь как сервисы. Остальное — **«инфраструктурные сервисы»** (AD, DNS, сеть, бэкапы) — описываются отдельным разделом, с нотацией: *«это сервисы для других сервисов, бизнес их не видит»*.
...
| **OLA** (Operational Level Agreement) | команда ИТ ↔ команда ИТ | сетевики обещают админам инфры |
...
SLA с бизнесом:    Email доступен 99.5%
   ...
   └─ SLO внутри ИТ:        99.7% (запас!)
         │
         ├─ OLA от сетевиков:  99.9% сетевая связность
         ├─ OLA от DBA:         99.8% БД доступна
         └─ UC от провайдера:   99.5% интернет
```

**SLA не может быть лучше, чем худший из его компонентов.** Если интернет от провайдера 99.5%, вы не можете обещать почту 99.9%. Это арифметика.
...
### Плановые работы — не downtime

В правильном SLA **плановое окно работ не считается downtime**. Иначе любая нормальная эксплуатация (патчи, апгрейды) убивает SLA.
...
**Что реально достижимо:** 99.5%-99.7% в рабочее время для on-premise Exchange с DAG, при условии хорошей сети и инфры.
...
Performance: операция чтения файла ≤500 МБ/сек, write ≤200 МБ/сек
...
Measurement: тестовое печатное задание раз в час на 5 ключевых принтеров каждой группы
...
**Подводные камни:**
- Это **самая ненадёжная часть инфры** в любой компании. Принтеры мрут, бумага кончается, кто-то застрял. SLA выше 95% — нереалистично.
...
Measurement: TCP-доступность OPC-сервера, ICMP до bridge-хоста
Performance: задержка обмена сообщениями ≤5 сек (95-й перцентиль)
...
Вариант A: 99.0% (∼7 часов downtime/месяц)
  Стоимость: текущая инфра, ничего не меняем
...
- ИТ-директор живёт в режиме «всё должно работать», бюджета на нормальную инфру нет
...
**Месяц 6-7:** Внутри ИТ согласовываем **SLO** (внутренние цели) на основе реальных данных. Не показываем бизнесу.
...
- [Глава 3 — Теги](03_tags_and_groups.md): теги `service`, `criticality`, `owner` — основа для расчёта SLA
