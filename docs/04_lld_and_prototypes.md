...
В вашем плане Zabbix для завода LLD прямо указан как часть стандартизации: интерфейсы, диски, очереди 1С и похожие повторяющиеся сущности нужно закрывать через LLD, а не руками. Это логично: в фазе 2 вы приводите шаблоны, группы, теги и severity к нормальной модели, и LLD там нужен именно для масштабирования шаблонов. 
...
Админ обычно не “работает с prototype’ом” каждый день. Он работает с уже созданным discovered item/trigger. Но архитектор мониторинга обязан понимать prototype’ы, потому что именно там задаётся массовое поведение:
