---
title: "Дашборды и отчётность: дизайн витрин"
description: "Раздел описывает дашборды и отчётность как витрину мониторинга: разные представления для дежурного, тимлида, владельца сервиса и руководства."
---
...
Дашборды и отчёты — это **витрина мониторинга**. То, что в итоге видят люди, принимающие решения: дежурный, тимлид, владелец сервиса, CIO. От качества витрины зависит, **сработает ли вся остальная конструкция**: сбор метрик, корреляция событий, runbooks — всё это бесполезно, если дежурный не понимает, что показано на экране, а CIO видит «зелёную галочку» вместо реальной картины.
...
- **Refresh rate.** Каждые 30 секунд для NOC, раз в час для CIO, раз в день для отчёта.
- **Контекст использования.** Большой экран в центре мониторинга, ноутбук в кафе или телефон в дороге.
...
- MSSQL: deadlock, длинные транзакции, blocking
...
- Один экран. Если не помещается — сокращать, а не добавлять вкладки

#### 5.  Прочие опасные для руководства

**Цель:** «работает ли ИТ всего свечного завода».
...
**Принципы:**
- Максимум 5-7 строк
- Формулировки на языке бизнес-сервиса, без низкоуровневых технических деталей
- Возможно — обновление раз в день (не в реальном времени)
- Часто — формат для скриншота на еженедельной планёрке у генерального

⚠️ **Согласуй с CIO и руководителями отделов**, нужен ли вообще такой дашборд. Иногда это политически сложно — кто-то может уведеть то, что не хотят показывать твои коллеги и руководство.
...
- Out-of-hours access (вход в административные системы вне рабочего времени)
...
### Таблица: как теги используются в дашбордах и виджетах

| Виджет / панель | Какие теги использует | Логика отбора |
|---|---|---|
| **Сетка состояния сервисов** | `service`, `env`, `criticality` | Показывает состояние сервисов в `env=prod`: например, `service=1c-erp`, `service=exchange`, `service=backup`. Агрегация строится по активным проблемам, synthetic checks или объектам Zabbix Services. |
| **Список инцидентов команды** | `owner`, `severity`, `env` | Показывает активные проблемы, за которые отвечает команда: например, `owner=it-apps`, `env=prod`, severity `High` и выше. |
| **Top-N по ёмкости** | `scope=capacity`, `component` | Показывает ресурсы, близкие к исчерпанию: диски, datastore, backup repositories, tablespaces. Сортировка — по утилизации, прогнозу заполнения или времени до исчерпания. |
| **Индикатор SLA/SLO** | `service` / `service tags` | Для штатного Zabbix SLA используется объект Zabbix Service и его service tags. Для внешней отчётности можно дополнительно группировать synthetic checks и problem events по `service=...`. |
| **Разбор по площадке** | `location`, `service`, `segment` | Показывает состояние сервисов и компонентов в конкретной локации: например, `location=dc-main`, затем детализация по `service` и `segment`. |
| **Статус резервного копирования** | `component=backup`, `scope=recovery`, `service` | Показывает проблемы backup jobs, repositories и RPO-проверок. Сортировка — по возрасту последнего успешного backup, severity и затронутому сервису. |
| **Состояние оборудования** | `component=hardware`, `location`, `os_family` | Показывает состояние iLO/iDRAC/IPMI/SNMP-сенсоров: питание, диски, RAID, вентиляторы, температура. Агрегация — по площадке и типу оборудования. |
| **Синтетические проверки** | `component=synthetic`, `service`, `env` | Показывает пользовательские проверки: login, отправка письма, открытие веб-публикации, чтение/запись файла. Фильтр — по сервису и среде. |
...
| Переход к детализации между дашбордами | Базовый | Гибкий: через Data Links, переменные и URL-параметры |
...
Grafana подключается к Zabbix через [Grafana Zabbix datasource plugin](https://github.com/grafana/grafana-zabbix) — плагин сообщества, поддерживаемый совместно сообществом и Grafana Labs (не официальный продукт Zabbix LLC). Умеет читать items, triggers, events с фильтрацией по тегам.
...
| Отчёт | Получатель | Периодичность | Источник данных | Доставка |
|---|---|---|---|---|
| **Ежедневная сводка дежурной смены** | Дежурная служба, тимлид эксплуатации | ежедневно, 09:00 | События Zabbix за последние сутки, активные проблемы, незакрытые инциденты | Email + канал Mattermost/Telegram |
| **Еженедельный операционный отчёт** | ИТ-команда, тимлиды направлений | понедельник, 09:00 | Агрегация событий и инцидентов за неделю, топ шумных алертов, изменения в мониторинге | Email + страница в Bookstack |
| **Ежемесячный отчёт по SLA/SLO** | CIO, руководители ИТ-команд, владельцы сервисов | 1-е число месяца | Zabbix Services + SLA, synthetic checks, Grafana-дашборды | Email с PDF-отчётом |
| **Ежемесячная управленческая сводка** | CIO для директора завода / руководства | 1-е число месяца | Сжатая версия SLA/SLO-отчёта: доступность ключевых сервисов, P1/P2-инциденты, динамика за месяц | Email от CIO |
| **Ежеквартальный capacity review** | CIO, руководители ИТ, владельцы платформ | раз в квартал | Тренды CPU/RAM/disk/storage/backup repositories/DB growth, прогноз исчерпания ресурсов | Встреча + презентация + архив в Bookstack |
| **Постинцидентный отчёт** | Команда эксплуатации, владельцы затронутого сервиса, CIO при P1 | по каждому P1, в течение 5 рабочих дней | Шаблон postmortem в Bookstack: timeline, impact, root cause, action items | Email участникам + архив в Bookstack |
| **Отчёт по качеству алертинга** | Тимлид эксплуатации, owner мониторинга | раз в 2 недели | Топ шумных триггеров, MTTA, acknowledge rate, события без реакции, алерты без runbook | Канал команды + задача на доработку |
...
Для большинства задач достаточно **Grafana panel render API + cron** на простом скрипте: одна задача cron в день рендерит нужные панели в PNG, склеивает в одну email и отправляет.
...
- [ ] Виджеты читают **теги** как основной источник (не жёстко заданный список имён хостов); явный список критичных компонентов допустим как часть service definition, если тегов недостаточно
...
- [ ] Ссылка на дашборд встроена в runbook через `{TRIGGER.URL}` или в Wiki
...
4. **Виджеты читают теги, не жёстко заданные имена.** Tag schema из [главы 3](03_tags_and_groups.md) — единый источник правды
