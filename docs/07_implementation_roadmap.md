...
Это классика — поставил, запустил, на выходе Excel/HTML с инвентарём. **Не пишешь код**, не парсишь API. Большинство админов начинают именно с них.
...
| **VMware инвентарь** | **RVTools** | https://www.robware.net/rvtools/ | Standalone .exe, читает vCenter, выдаёт Excel со всеми VM, дисками, снапшотами, сетями. Любимый инструмент vmware-админов |
...
| IT-Infra админы | Infra/* + Locations/* | Infra/* | manage hosts, items |
...
- ❌ **Пытаться унифицировать всё.** Завод — разнородная среда. Где-то останутся легаси-шаблоны и легаси-теги. Это норма.
...
5. **Документация — там, где её ищут пользователи.** Не там, где удобно класть админу. Если у них Confluence — клади туда. Если Bookstack — туда. Если папки в DFS — туда, как ни больно.
...
- **Отдельный технический пользователь** для каждого сервиса (не админ!), с минимальными правами
...
## Эскалация
- 15 минут не починилось → звонок старшему 1С-админу (Application Owner, <phone>)
