...
4. **Никакого нового мониторинга и инфраструктуры.** Хочется поднять Netbox или CMDB? Нельзя — это полугодовой проект сам по себе. Read-only инструменты инвентаризации (RVTools, ADRecon, zabbix-cli, Oxidized) — нормально, это не инфраструктура. OCS Inventory требует установки агентов на хосты — это уже не read-only, откладывается. Excel/Confluence/Bookstack — что уже есть
...
| Best Practices (нативный док) | https://www.zabbix.com/documentation/current/en/manual/best_practices |
...
2. **Не Big Bang.** Меняем волнами: схему согласовали → пилот 50 хостов → раскатываем на остальные.
...
```
`last(/host/key,5m)` возвращает последнее значение, собранное не позже чем 5 минут назад — это не confirmation period. Для подтверждения устойчивого нарушения используй `min()` или `avg()`. Минимальная длительность — приём от 80% флапа.
...
> ⚠️ Dependencies — это связи между trigger'ами; для topology-зависимостей между разными хостами их нужно поддерживать осознанно, вручную или через API/шаблоны. У trigger prototypes есть отдельные ограничения по зависимостям. Не используйте синтетический чек как root-cause для business-компонентов — это маскирует инфраструктурные проблемы. Для сервисной иерархии и бизнес-представления рассматривайте Zabbix Services + problem tags (см. [главу 1](01_service_not_host.md)).
...
## Process — как раскатывать изменения без срыва
...
- ❌ **Переписывать встроенные шаблоны Zabbix.** При апгрейде версии Zabbix шаблоны не обновляются автоматически, но при ручном импорте новой версии официального шаблона прежние локальные правки можно потерять. В production — только свои клоны или управляемые производные шаблоны.
...
Zabbix web scenarios (httptest) покрывают HTTP/HTTPS-шаги: login, переходы, проверка контента. Большая ценность: ловит проблемы, которых не видят item-чеки.
...
- **Изоляция:** технический пользователь синтетики не должен ломать продакшн-данные при ошибке
...
- **Не плодить тикеты-дубли.** `eventid` в Zabbix уникален для каждого problem event — новое срабатывание = новый eventid. Как correlation key для ITSM используй `triggerid` (стабильный идентификатор триггера). Нужна также политика группировки: что делать если тикет уже открыт при повторном срабатывании — переоткрыть, добавить комментарий или проигнорировать
...
- [ ] **Дашборды по аудиториям** созданы и в эксплуатации (NOC, per-service, CIO — приоритет)
- [ ] **Synthetic monitoring** для ключевых сервисов запущен (2-3 минимум, 5-8 — цель)
- [ ] **Runbooks library** — все Disaster покрыты; High — по мере возможности (30+ — цель, не требование)
