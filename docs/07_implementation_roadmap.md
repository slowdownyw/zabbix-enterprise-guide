---
title: "90-дневная дорожная карта внедрения"
description: "Практический 90-дневный roadmap помогает войти в наследуемую инсталляцию Zabbix, снизить технический долг и привести мониторинг к управляемой модели."
---

!!! info "🟡 Статус: Design draft · v0.1"
    Подход правильный, но конкретный код, конфиги или пороги требуют валидации в вашем окружении.

# 07. 90-дневная дорожная карта внедрения

План на 3 месяца для DevOps/инженера, который входит в проект с **наследуемой инсталляцией Zabbix с накопленным техническим долгом** и задачей привести её в управляемое состояние. Подходит и для нового проекта, но основной упор сделан на уже существующую систему.
...
## Реалии РФ: что работает, что нет
...
В Veeam бэкапится, но в Zabbix не виден → объект защищается, но не наблюдается. Мы не можем контролировать то, чего не видим.
---
...
Что болит: фоновые задачи виснут, узнаём от пользователей
...
ID: PAIN-007
Источник: интервью с дежурным + журнал инцидентов

Проблема:
Алерты по дисковому пространству приходят ночью на серверы,
где штатный скрипт ротации логов обычно освобождает место за 3–5 минут.
Дежурный знает об этом и фактически не реагирует на такие срабатывания.

Влияние:
Формируется усталость от алертов: дежурный перестаёт доверять каналу
уведомлений и может пропустить настоящий критический инцидент.

Категория: Alert hygiene
Приоритет: высокий

Решение в фазе 2:
- пересмотреть пороги и severity для этих триггеров;
- добавить confirmation period: алертить только если проблема держится дольше N минут;
- добавить hysteresis/recovery expression, чтобы убрать флаппинг;
- для серверов с автоматической ротацией логов перевести кратковременное превышение в Warning/Information;
- оставить Disaster/High только для случаев, когда диск остаётся заполненным после работы ротации или прогноз заполнения указывает на реальный риск.
...
**Это документ для CIO/ИБ, не для общего распространения.** Прямо так и пометить: `Конфиденциально: только CIO+ИБ`.
...
Не всё, что выглядит как «синтетика», является httptest. Файловые шары, принтеры, Kerberos, 1С тонкий клиент — это agent checks, внешние скрипты или UserParameter, а не web сценарии. Таблица ниже объединяет оба подхода.
...
### Финальные артефакты проекта

Документы, ссылка на главную страницу Bookstack:

# Руководство по эксплуатации мониторинга Свечной заводик IT
...
Краткое описание целевой архитектуры мониторинга: источники данных, Zabbix Server, Zabbix Proxy, база данных, Grafana, Service Desk, каналы оповещения и контур реагирования.

## Схема тегов и групп хостов

[ссылка на документ]

Правила использования host groups и tags:
- группы хостов — для технической структуры и разграничения прав доступа;
- теги — для сервисной классификации, маршрутизации алертов, дашбордов, SLA/SLO-отчётности и корреляции событий.

## Политика критичности и эскалации

[ссылка на документ]

Описание уровней severity, правил маршрутизации уведомлений, времени реакции, порядка эскалации и связи severity с инцидентами в Service Desk.

## Каталог дашбордов

- Оперативный экран дежурной смены / NOC: [ссылка]
- Дашборды по сервисам: [ссылка]
- Дашборды команд эксплуатации: [ссылка]
- Дашборд руководителя эксплуатации: [ссылка]
- Дашборд CIO / ИТ-директора: [ссылка]
- SLA/SLO-отчётность: [ссылка]

## Каталог runbook'ов

[ссылка на список]

Единый каталог инструкций по реагированию на типовые события мониторинга: P1/P2-инциденты, отказ сервисов, деградация компонентов, проблемы с агентами, backup, сетью, БД и прикладными системами.

## Эксплуатационные инструкции

- Добавление нового хоста в мониторинг
- Подключение нового шаблона
- Изменение порога триггера
- Разбор сработавшего алерта
- Создание и обновление runbook'а
- Создание maintenance window
- Вывод хоста из мониторинга
- Проверка корректности тегов и групп
- Подготовка внепланового отчёта
- Проведение постинцидентного разбора

## Дежурства и on-call

[ссылка]

Правила дежурной ротации:
- кто принимает первичные алерты;
- кто является резервным дежурным;
- когда подключается L2/L3;
- когда эскалировать руководителю эксплуатации или CIO;
- как фиксировать принятие события в работу, комментарии дежурного и передачу смены;

## Календарь отчётности

[ссылка или таблица]

Описание регулярных отчётов:
- ежедневная сводка дежурной смены;
- еженедельный отчёт по инцидентам и шумным алертам;
- ежемесячный SLA/SLO-отчёт;
- ежеквартальный capacity review;
- отчёты по postmortem для P1/P2-инцидентов.

## Ответственные и зоны владения

[ссылка]

Матрица ответственности по сервисам, системам и компонентам:
- бизнес-владелец сервиса;
- ИТ-владелец сервиса;
- команда эксплуатации;
- ответственный за шаблон мониторинга;
- ответственный за runbook;
- канал эскалации.
...
Порядок внесения изменений в мониторинг:
- изменение шаблонов;
- изменение severity;
- изменение trigger expressions;
- изменение тегов и групп;
- добавление новых actions;
- изменение маршрутизации уведомлений;
- изменение дашбордов и отчётов.

Все значимые изменения в системе мониторинга должны проходить через процедуру управления изменениями: оформляется запрос на изменение, оценивается влияние, фиксируется владелец, согласуется выполнение, задаются план проверки и план отката. Не забывайте, внося изменение в систему, вы не меняете SLA!
...
Список ограничений, временных решений и задач, не закрытых в рамках текущей фазы:
- шумные триггеры, требующие ревизии;
- хосты без полного набора тегов;
- сервисы без runbook'ов;
- непокрытые синтетикой бизнес-сервисы;
- неавтоматизированные отчёты;
- устаревшие шаблоны;
- зоны без владельца;
- элементы мониторинга, требующие переработки в следующих фазах.
...
- ❌ **Перекладывать всё в Grafana одним большим переносом.** Часть остаётся в Zabbix — это нормально.
- ❌ **Подписывать SLA с бизнесом в первый год.** Это политика, требует подготовки.
- ❌ **Делать runbook формально.** Лучше 20 хороших runbook, чем 100 формальных. Формальные runbook никто не будет использовать.
...
1. **Демо «как было / как стало»** для CIO и команды
   - Метрики до/после: число алертов в день, MTTR, покрытие хостов %, число активных триггеров
...
- ✅ Покрытие хостов выросло с условных 70% до 95%+
