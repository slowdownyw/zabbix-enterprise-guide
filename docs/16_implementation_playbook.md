...
| **Application owners** | 1С, Exchange, MSSQL, Veeam, ERP/CRM команды | Прикладные метрики, критичность, диагностика, инструкции runbook |
| **NOC / on-call lead** | Дежурная служба / L1 lead | Проверка процесса реакции, escalation, acknowledgement, передача в эксплуатацию |
...
| Представитель дежурной службы | Тот, кто реально получает алерты |
...
Sizing нельзя делать только после rollout. Правильный подход — в два этапа:

- **Sizing baseline** (Phase 0–1): оценочные цифры по хостам, items, NVPS, retention — утвердить до Phase 3/pilot.
- **Уточнение sizing** (после pilot): скорректировать по фактическому item rate, preprocessing нагрузке, web scenarios, LLD cardinality, queue и DB write latency.
...
| Gate 5 | После передачи в эксплуатацию | Команда может жить без автора внедрения? |
...
- [ ] Еженедельный разбор алертов запущен.
- [ ] Zabbix self-monitoring настроен и включает: internal items, queue depth, cache hit rate, poller/preprocessor busy %, DB write latency, history syncers, unsupported items count, proxy last seen.
