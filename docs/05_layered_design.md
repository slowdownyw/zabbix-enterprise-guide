...
- 🧪 **Тестовые стенды = production-шаблоны минус один слой.** Тестовый сервер 1С — те же шаблоны, кроме `Synthetic`. Один список присоединённых шаблонов = одна разница.
...
**Правильная формулировка:** «к хосту присоединяется набор шаблонов, каждый отвечает за свою грань». Не «`Plant: 1C App Server` наследует от `Plant: Windows Base`».
...
## 5. Семантика события — как оно приобретает смысл
