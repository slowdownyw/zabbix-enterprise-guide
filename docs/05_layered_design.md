...
- **`Plant: Common Policy`** — только macros, conventions, технические теги класса. Никаких items, минимум зависимостей. **Предупреждение:** если несколько шаблонов определяют одинаковые macros с разными значениями, приоритет определяется порядком присоединения и уровнем (host > template). Задокументируйте правила macro precedence и ведите drift-контроль, чтобы не получить неожиданного значения порога.
