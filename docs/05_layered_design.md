...
1. **Linked templates** — к хосту можно привязать несколько шаблонов одновременно
2. **Template links** — шаблоны могут ссылаться на другие шаблоны (template-to-template linking)

Это не parent-child с override-логикой. Это **композиция**: к хосту привязывается стек шаблонов, каждый отвечает за свою часть.
...
- 🧪 **Тестовые стенды = production-шаблоны минус один слой.** Тестовый сервер 1С — те же шаблоны, кроме `Synthetic`. Один список привязанных шаблонов = одна разница.
...
**Правильная формулировка:** «к хосту привязывается набор шаблонов, каждый отвечает за свою грань». Не «`Plant: 1C App Server` наследует от `Plant: Windows Base`».

Можно привязать шаблон к шаблону (template-to-template linking), но **не для имитации наследования**, а для агрегации: «`Plant: Role - 1C App Node` включает `Plant: Windows Services Base` для удобства привязки роли одной строкой». Это утилитарное использование, не архитектурный паттерн.
...
- **`Plant: Common Policy`** — только macros, conventions, технические теги класса. Никаких items, минимум зависимостей. **Предупреждение:** если несколько шаблонов определяют одинаковые macros с разными значениями, приоритет определяется порядком привязки и уровнем (host > template). Задокументируйте правила macro precedence и ведите drift-контроль, чтобы не получить неожиданного значения порога.
...
Перед привязкой шаблонов к хосту — пройдите по этому списку:
...
- [ ] Привязан Base-шаблон ОС
- [ ] Привязан Role-шаблон (если есть)
- [ ] Привязаны Application-шаблоны (по числу прикладных систем)
- [ ] Привязан Synthetic-шаблон для критичных сервисов
