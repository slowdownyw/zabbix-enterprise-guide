...
Template         ← "конвенции этого класса систем"
   ↑
Host             ← "кто этот объект и чей он"
```

Все теги со всех уровней **наследуются в проблемное событие**. Это значит: в момент срабатывания у события есть **полный набор тегов** со всех слоёв сразу. Это и есть точка, в которой склеиваются «кто», «что» и «как реагировать».
...
> **Если посмотреть на problem event и не понять без расследования "что упало, у кого, какой сервис, что делать" — дизайн сломан.**
...
> **Правило 1.** Host tags отвечают на вопрос **«кто этот объект и чей он»**: env, location, segment, owner, criticality, service, os_family, role.
...
## 6. Worked example: `1c-app-warehouse-01`
...
Это и делает книгу методикой, а не сборником шаблонов: единый язык, переиспользуемый везде.
...
| Item/Trigger tags | «Что произошло» | «Кто владелец» |
...
Сводка ошибок именно в этом дизайнерском слое. Общие anti-patterns мониторинга — в [главе 15](15_antipatterns.md).
...
Перед линковкой шаблонов к хосту — пройдите по этому списку:
...
4. **Матрица тегов:** host tags = «кто и чей», item/trigger tags = «что произошло»
