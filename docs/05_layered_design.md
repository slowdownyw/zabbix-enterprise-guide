---
title: "Многоуровневая модель: шаблоны, теги и смысл событий"
description: "Глава связывает сервисную модель, severity, теги, host groups и LLD в многоуровневый дизайн, где каждое событие Zabbix получает понятный смысл."
---
...
Но при первой же попытке спроектировать реальный шаблон возникает вопрос: **а как они складываются вместе?** В каком шаблоне жить какой метрике? Какой тег вешать на хост, какой — на триггер? Что окажется в событии в момент срабатывания? Куда пойдёт алерт?
...
Слово «слой» в обсуждениях мониторинга используется в **четырёх разных смыслах**, и без разнесения их книга превращается в кашу:

| # | Тип слоя | Что описывает | Где обсуждается |
|---|---|---|---|
| 1 | **Слои мониторинга** (*monitoring layers*) | *Что* измеряем (Hardware → Business) | глава 6 (Архитектура) |
| 2 | **Слои композиции шаблонов** (*template composition layers*) | *Как* собираем шаблоны (Base / Role / App / Synthetic) | эта глава, глава 13 |
| 3 | **Слои размещения тегов** (*tag placement layers*) | *Где* живёт тег (Host / Template / Item / Trigger / Service) | глава 3, эта глава |
| 4 | **Слои события и маршрутизации** (*event / routing layers*) | *Что* приходит в событие и *куда* оно идёт | эта глава |
...
- Слой мониторинга: **OS**
- Слой композиции шаблонов: реализуется через `Plant: Windows Base` (присоединённый шаблон)
- Размещение тегов: тег `component=filesystem` — на item prototype, тег `service=1c-erp` — на host
- Событие и маршрутизация: при срабатывании событие получает оба тега и маршрутизируется в команду `it-apps` потому что `service=1c-erp`
...
### 2.1 Слои мониторинга — что измеряем
...
### 2.2 Слои композиции шаблонов — как собираем шаблоны

В Zabbix **нет ООП-наследования** в строгом смысле. Есть:

1. **Linked templates** — к хосту можно присоединить несколько шаблонов одновременно
2. **Template links** — шаблоны могут ссылаться на другие шаблоны (template-to-template linking)

Это не parent-child с override-логикой. Это **композиция**: к хосту присоединяется стек шаблонов, каждый отвечает за свою часть.
...
Например: `Plant: Role - MSSQL Server` — это «на хосте установлен MSSQL, мониторим сервис, порт 1433, базовые wait stats». `Plant: MSSQL for 1C` — это «на этом экземпляре живёт 1С: deadlocks для базы 1С, размер tempdb, специфика её рабочей нагрузки».
...
- 🔄 **Изоляция обновлений.** Можно обновить `Plant: 1C App Server` без риска сломать OS-метрики.
- 🧪 **Тестовые стенды = боевые шаблоны минус один слой.** Тестовый сервер 1С — те же шаблоны, кроме `Synthetic`. Один список присоединённых шаблонов = одна разница.
- 📊 **Организационная ответственность по композиции.** В Zabbix права строятся через user roles и host groups, не через шаблоны напрямую. Но шаблонная декомпозиция отражает организационную ответственность в Git/review: команда DBA отвечает за `Plant: MSSQL for 1C`, команда 1С — за `Plant: 1C App Server`. Это помогает контролировать изменения через code review, а не через UI-права.
...
### 2.3 Слои размещения тегов — где живёт тег
...
Все теги со всех уровней **наследуются в событие со статусом PROBLEM**. Это значит: в момент срабатывания у события есть **полный набор тегов** со всех слоёв сразу. Это и есть точка, в которой склеиваются объект, проблема и способ реакции.

Подробная матрица — в разделе 4 ниже.

### 2.4 Слои события и маршрутизации — что приходит в событие

Это самое важное измерение, которое обычно проскакивают: **дизайн всех остальных слоёв осмыслен только потому, что в итоге собирается осмысленное событие со статусом PROBLEM**.
...
> **Если посмотреть на событие со статусом PROBLEM и не понять без расследования, „что упало, где, какой сервис затронут, что делать" — дизайн сломан.**

Это и есть метрика качества проектирования. Если событие содержит `1c App Server is down on srv-1cd5-04` и больше ничего — придётся гуглить, в каком цеху эта система, кому звонить, есть ли runbook. Если событие содержит `service=1c-erp env=prod owner=it-apps location=dc-main component=rphost impact=service-degradation runbook=rb-1c-rphost-down` — дежурный действует за 30 секунд.

---

## 3. Композиция, а не наследование
...
|---|---|
| Один родитель, один потомок | Несколько шаблонов на хост |
| Переопределение методов | Нет переопределения; items с одинаковым key конфликтуют |
| Иерархия глубины N | Плоский список присоединённых шаблонов на хост |
| Полиморфизм | Нет полиморфизма |

**Правильная формулировка:** «к хосту присоединяется набор шаблонов, каждый отвечает за свою часть». Не «`Plant: 1C App Server` наследует от `Plant: Windows Base`».

Можно присоединить шаблон к шаблону (template-to-template linking), но **не для имитации наследования**, а для агрегации: «`Plant: Role - 1C App Node` включает `Plant: Windows Services Base` для удобства присоединения роли одной строкой». Это утилитарное использование, не архитектурный паттерн.

### Где живут общие UserParameter и макросы
...
- **`Plant: Common Policy`** — базовый policy-шаблон без items и triggers. Содержит только общие macros, соглашения и технические теги класса: например, `monitoring_profile`, `template_family`, `policy_version`. Его задача — задавать общую политику мониторинга, а не собирать метрики.

  **Важно:** не используйте `Plant: Common Policy` как свалку для всех порогов. Если один и тот же macro определён на хосте, в нескольких шаблонах и глобально, Zabbix выберет значение по своей иерархии precedence: host → напрямую привязанные templates → templates следующих уровней → global macros. Если одинаковый macro задан в нескольких шаблонах одного уровня, победит значение из шаблона с меньшим template ID, а не «последний присоединённый» шаблон.

  Поэтому общие macros нужно документировать, избегать дублей между шаблонами одного уровня и регулярно проверять drift: иначе один и тот же trigger prototype может получить неожиданный порог на разных хостах.
- **`Plant: Agent Health`** — мониторинг самого Zabbix-агента: доступность агента, актуальность active checks, версия агента.
...
| `runbook` | **Trigger** или URL в description | `runbook=rb-1c-rphost-down` | Ссылка на конкретный сценарий реагирования |
...
### Где не стоит использовать тег `service`

Тег `service` — **тег уровня хоста или Zabbix Service**, не триггера. На триггер его задавать стоит **только как осознанное исключение**, когда событие на этом хосте относится к **другому** сервисному контексту, чем сам хост.

Пример исключения: на сервере 1С работает агент Veeam, проверяющий бэкапы. Сам хост — это `service=1c-erp`. Но триггер «бэкап-задание не выполнилось > RPO» — это уже `service=backup` контекстно. В этом случае на триггер ставится отдельный `service=backup`, а на хост остаётся `service=1c-erp`. Но это редкий случай, требующий осознанного решения.

Обычно же — `service` приходит с хоста или Zabbix Service, а на триггер хватает `component`, `scope`, `impact`.

---

## 5. Семантика события: как собираются теги

Когда триггер срабатывает, Zabbix создаёт **событие со статусом PROBLEM**. К нему **наследуются теги со всех слоёв**:
...
В шаблонах на items и prototypes висят **технические теги** — что именно измеряется:
...
### Шаг 6. Итоговое событие PROBLEM

Когда триггер «Synthetic login failed 3 times» сработает, в Zabbix создастся событие со статусом PROBLEM и **всеми тегами одновременно**:
...
## 7. Где применяется эта модель

Самое важное в многоуровневой модели — **она не для красоты шаблонов**. Она нужна для того, чтобы **все остальные подсистемы Zabbix говорили на одном языке**.
...
> Дашборды, маршрутизация алертов, SLA-отчёты и сервисное представление **не должны изобретать отдельную структуру**. Они используют те же теги и слои, которые задаёт дизайн шаблонов.
...
В результате получается единая методика, а не набор разрозненных шаблонов: один язык тегов и слоёв применяется в шаблонах, дашбордах, маршрутизации, SLA и отчётах.
---
...
❌ **Вешать `service=`, `env=`, `location=` на item или триггер.** Это host-level теги. Если поставили на item — наследуются в событие, конфликтуют с host-level, путаются в action.
...
❌ **Ручное тегирование объектов, обнаруженных через LLD.** Если обнаруженный интерфейс получает теги вручную — при следующем цикле LLD они слетят. Теги задаются на прототипе.

❌ **Runbook только в Wiki, без ссылки в триггере.** Дежурный в 3 ночи не найдёт. URL обязан быть в `{TRIGGER.URL}` или description.
...
Перед присоединением шаблонов к хосту — пройдите по этому списку:
...
- [ ] К хосту присоединён OS Base template
- [ ] К хосту присоединён Role template (если есть выраженная роль)
- [ ] К хосту присоединены Application templates (по числу прикладных систем)
- [ ] К хосту присоединён Synthetic template для критичных сервисов
...
- [ ] Выражение триггера в актуальном синтаксисе Zabbix (`last(/host/key)>X`)
...
- [ ] Тестовое событие (закрытое вручную) маршрутизируется в правильный канал
...
1. **Четыре измерения слоёв** ортогональны: мониторинг, композиция, размещение тегов, событие и маршрутизация
2. **Композиция, а не наследование.** В Zabbix несколько шаблонов на хост, не parent-child
...
5. **Событие приобретает смысл** через сумму тегов со всех уровней. Цель дизайна — осмысленное событие со статусом PROBLEM
6. **Многоуровневая модель применяется повторно** в дашбордах, action routing, SLA, отчётах. Не дублируется
