...
4. Прогоните на 1–2 хостах в течение 1–2 недель **прежде чем раскатывать массово**
...
При доработке корпоративных шаблонов эти теги добавляются поверх host-тегов из [главы 3](03_tags_and_groups.md).
...
3. **UserGate** — MIB скачивается из консоли, разворачивается community-шаблон, проверяются traps.
4. **1C** — самое трудоёмкое. `slothfk` используется как база, UserParameter'ы дорабатываются под локальную конфигурацию. Плановый объём работ — 2–3 дня.
